...
class EnhancedIPTVPlayerBackendTest(unittest.TestCase):
    def setUp(self):
        ...
        self.enhanced_m3u_content = """#EXTM3U
...
"""
    ...
    def test_01_enhanced_m3u_parser(self):
        """Test enhanced M3U parser with real streaming URLs"""
        print("\n=== Testing Enhanced M3U Parser with Real Streaming URLs ===")
        
        # Create a file-like object with enhanced M3U content
        m3u_file = BytesIO(self.enhanced_m3u_content.encode('utf-8'))
        
        # Prepare the multipart/form-data request
        files = {
            'file': ('enhanced_playlist.m3u', m3u_file, 'application/octet-stream')
        }
        data = {
        ...
        data = response.json()
        self.assertEqual(data["name"], "Enhanced Test Playlist")
        self.assertEqual(data["channel_count"], 3)
        
        # Save playlist ID for later tests
        self.playlist_ids.append(data["id"])
        print(f"✅ Enhanced M3U parser working - Created playlist with ID: {data['id']}")
        ...
        for channel in channels:
            if channel["name"] == "NASA Live":
                ...
                print("✅ NASA Live channel correctly parsed")
                ...
                print("✅ RT Documentary channel correctly parsed")
                ...
                print("✅ Big Buck Bunny MP4 channel correctly parsed")

    def test_02_encoding_support(self):
        """Test M3U parser with different character encodings"""
        print("\n=== Testing M3U Parser Encoding Support ===")
        
        # Create a file-like object with Latin-1 encoded content
        m3u_file = BytesIO(self.latin1_m3u_content.encode('latin-1'))
        
        # Prepare the multipart/form-data request
        files = {
            'file': ('latin1_playlist.m3u', m3u_file, 'application/octet-stream')
        }
        data = {
            'name': 'Latin-1 Encoded Playlist'
        }
        ...
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["name"], "Latin-1 Encoded Playlist")
        self.assertEqual(data["channel_count"], 2)
        
        # Save playlist ID for later tests
        self.playlist_ids.append(data["id"])
        print(f"✅ Latin-1 encoding support working - Created playlist with ID: {data['id']}")
    ...
    def test_03_streaming_url_validation(self):
        """Test validation of various streaming protocols"""
        print("\n=== Testing Streaming URL Validation ===")
        ...
        # Prepare the multipart/form-data request
        files = {
            'file': ('protocols_playlist.m3u', m3u_file, 'application/octet-stream')
        }
        data = {
